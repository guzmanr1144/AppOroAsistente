...
# ══════════════════════════════════════════════════════════════
# CSS — COMPLETO Y RESTAURADO
# ══════════════════════════════════════════════════════════════
@st.cache_data(show_spinner=False)
def _get_all_css(tema_key="noche"):
    _T = {
        "noche": {"bg1":"#0a0e1a","bg2":"#0d1222","bg3":"#10162a","card":"#141828","card2":"#1a2035","borde":"#2a3560","borde2":"#364480","acento1":"#6b83f8","acento2":"#8fa0ff","acento3":"#4f6ef7","titulo_grad":"linear-gradient(135deg,#6b83f8,#a78bfa,#6b83f8)","texto":"#e8ecff","texto2":"#a8b8f0","texto3":"#5060a0","sombra":"rgba(107,131,248,0.2)","sombra2":"rgba(107,131,248,0.08)"},
        "carbon": {"bg1":"#111418","bg2":"#161b22","bg3":"#1c2330","card":"#1e2530","card2":"#242e3d","borde":"#2d3f55","borde2":"#3a5070","acento1":"#10b981","acento2":"#34d399","acento3":"#6ee7b7","titulo_grad":"linear-gradient(135deg,#10b981,#06b6d4,#10b981)","texto":"#d8f0e8","texto2":"#90c8b0","texto3":"#3a6050","sombra":"rgba(16,185,129,0.2)","sombra2":"rgba(16,185,129,0.08)"},
        "cosmos": {"bg1":"#0d0818","bg2":"#120d22","bg3":"#18102c","card":"#1a1030","card2":"#201540","borde":"#3a2060","borde2":"#502888","acento1":"#a78bfa","acento2":"#c4b0ff","acento3":"#7c3aed","titulo_grad":"linear-gradient(135deg,#a78bfa,#f472b6,#a78bfa)","texto":"#f0e8ff","texto2":"#c0a8f0","texto3":"#705898","sombra":"rgba(167,139,250,0.2)","sombra2":"rgba(167,139,250,0.08)"}
    }
...
_TXT = {
    "es": {
        "analizar":"⚡ Analizar","evaluar":"🔍 Evaluar","cargando":"📖 Cargando...","procesando":"🧠 Procesando...",
        "chat_placeholder":"✍️ Escribe un cambio o pregunta...","confirmar":"✅ Confirmar","cancelar":"❌ Cancelar",
...
# ══════════════════════════════════════════════════════════════
# MOTORES DE TRABAJO (RESTAURADOS)
# ══════════════════════════════════════════════════════════════
try:
    genai.configure(api_key=st.secrets["LLAVE_GEMINI"])
except: st.error("Falta LLAVE_GEMINI en secrets")
...
if st.session_state.texto_extraido:
    ...
    if st.session_state.preview_cambio:
        ...
        with c1:
            if st.button(T("confirmar")):
                tipo, bytes_in = st.session_state.archivo_tipo, (st.session_state.cambios_aplicados or st.session_state.archivo_bytes)
    ...
    with ba:
        if st.button(T("analizar")):
            r = llamar_ia(f"Resume este doc en JSON: {{'titulo':'','resumen_ejecutivo':''}}\n\nDoc: {texto_activo[:2000]}")
            st.session_state.resumen_data = extraer_json_seguro(r)
    with be:
        if st.button(T("evaluar")): st.info("🔍 Evaluación en desarrollo...")
    ...
    if entrada:
        ...
        with st.spinner(T("procesando")):
            r = llamar_ia(f"Asistente editorial. Devuelve SOLO JSON array: [{{'buscar':'', 'reemplazar':''}}]. Instrucción: {entrada}. Contexto: {texto_activo[:2000]}")
            nuevos = extraer_json_seguro(r, es_lista=True)
            if nuevos: st.session_state.preview_cambio = nuevos
            else: st.session_state.historial_chat.append({"rol":"Asistente", "texto":"No entendí el cambio o no encontré el texto."})
        st.rerun()
    ...
    st.markdown('<div style="text-align:center;padding:3rem 1rem">📂 Sube un archivo para empezar</div>', unsafe_allow_html=True)
